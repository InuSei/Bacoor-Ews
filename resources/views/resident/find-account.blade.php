...
<head>
    ...
    <title>Find Your Account - CDRRMO Bacoor</title>
    ...
    <div class="w-full max-w-md bg-white rounded-3xl shadow-xl border border-gray-200 overflow-hidden flex flex-col justify-between relative min-h-[600px]">
        ...
        <div class="px-8 flex-1 flex flex-col justify-center">
            <div class="mb-6">
                ...
                <p class="text-xs text-gray-500 mt-1 font-semibold">Enter your mobile number.</p>
            </div>

            <form method="POST" action="{{ route('resident.find-account.post') }}" class="space-y-4">
                @csrf
                ...
                <div>
                    <input 
                        ...
                        class="w-full h-12 px-4 rounded-xl border {{ $errors->has('phone_number') ? 'border-red-500 bg-red-50' : 'border-gray-200 bg-gray-50' }} text-slate-900 text-sm font-bold focus:outline-none focus:border-blue-500 focus:bg-white transition-all tracking-wide"
                        required
                    >
            ...
            <div class="text-center mt-6">
                <a href="{{ route('resident.find-account-email') }}" class="text-xs font-bold text-slate-700 hover:text-blue-600 transition-colors inline-block text-decoration-none">
