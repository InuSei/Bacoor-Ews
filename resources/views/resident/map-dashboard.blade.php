...
<body class="bg-slate-100 font-sans relative w-full h-full text-gray-800">
        ...
        <div class="bg-white/95 backdrop-blur-md rounded-2xl shadow-xl border border-gray-200 p-3.5 flex items-center justify-between gap-3 transition-all focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100">
            <div class="flex items-center gap-3 flex-1">
                ...
                <input 
                    type="text" 
                    id="mapSearchInput"
                    placeholder="Search Barangay (e.g., Talaba)..." 
                    class="bg-transparent border-none text-slate-800 text-sm font-bold w-full focus:outline-none placeholder-gray-400"
                    onfocus="toggleSearchDropdown(true)"
                >
            </div>
            ...
            <div class="flex items-center gap-3 border-l border-gray-200 pl-3 relative">
                <div 
                    ...
                    class="w-8 h-8 rounded-full bg-slate-100 border border-gray-200 hover:bg-slate-200 flex items-center justify-center shadow-xs cursor-pointer transition-colors select-none overflow-hidden"
                >
                ...
                <div 
                    ...
                    class="hidden absolute top-11 right-0 w-48 bg-white/95 backdrop-blur-md rounded-xl shadow-2xl border border-gray-200 overflow-hidden z-[10000]"
                >
                    <div class="px-4 py-2.5 bg-gray-50 border-b border-gray-100 text-[10px] font-black text-gray-400 uppercase tracking-wider">
                        Account Settings
                    </div>
    ...
    <button id="mobileInfoToggleBtn" onclick="toggleMobileInfoCard()" class="md:hidden absolute bottom-6 left-1/2 -translate-x-1/2 z-[9998] bg-[#0F2942] text-white px-6 py-3.5 rounded-full shadow-[0_10px_25px_rgba(0,0,0,0.3)] font-bold text-xs flex items-center gap-2 border border-slate-700 transition-transform active:scale-95 whitespace-nowrap">
        <span class="text-sm leading-none">🚨</span> Status & Emergency
    </button>
    ...
    <div id="bottomInfoCard" class="absolute bottom-0 left-0 w-full md:bottom-6 md:left-6 md:w-[420px] z-[9999] bg-white/95 backdrop-blur-md rounded-t-3xl md:rounded-2xl shadow-[0_-15px_40px_rgba(0,0,0,0.15)] md:shadow-2xl border-t md:border border-gray-200 transition-transform duration-300 transform translate-y-full md:translate-y-0 p-6 pt-8 md:pt-6">
        ...
        <div class="md:hidden absolute top-0 left-0 w-full h-10 flex items-center justify-center cursor-pointer" onclick="toggleMobileInfoCard()">
        ...
        <div id="defaultMenuSection">
            ...
            <div class="mt-4 border border-gray-100 rounded-xl bg-gray-50/50 overflow-hidden text-xs">
                <div class="bg-white px-4 py-2.5 border-b border-gray-100 font-bold text-slate-800 flex items-center gap-2">
                    <img src="{{ asset('images/phone-call.png') }}" alt="Emergency" class="w-4 h-4 object-contain">
            ...
            <div class="flex flex-col gap-3">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <div id="badgeAlertIndicatorBlock" class="inline-block mt-1 px-3 py-1 text-[10px] font-black text-white uppercase rounded shadow-xs">
                            CRITICAL
                        </div>
                    </div>
                    <button onclick="resetToBaselineView()" class="text-gray-400 hover:text-slate-600 font-bold text-xs bg-gray-100 hover:bg-gray-200 px-2 py-1 rounded-md transition-colors cursor-pointer">
                        Clear Map ✖
                    </button>
                </div>
                <p id="evacuationInstructionLabelText" class="text-xs font-bold text-slate-600 leading-relaxed border-l-4 border-red-500 pl-3 bg-red-50/50 py-2.5 rounded-r-xl">
                    Loading localized environmental infrastructure vector maps...
                </p>
            </div>
        </div>
    ...
    <script>
        ...
        const locationCoordinatesMatrix = {
            cdrrmo_hq: { lat: 14.4314, lng: 120.9463 }, 
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            L.marker([locationCoordinatesMatrix.cdrrmo_hq.lat, locationCoordinatesMatrix.cdrrmo_hq.lng])
             .bindPopup("<b>CDRRMO Headquarters</b><br>Bacoor Government Center").addTo(map);
        ...
        function toggleMobileInfoCard() {
            ...
            if (card.classList.contains('translate-y-full')) {
                card.classList.remove('translate-y-full'); btn.classList.add('hidden');
            } else {
                card.classList.add('translate-y-full'); btn.classList.remove('hidden');
            }
        }

        function forceOpenMobileInfoCard() {
            if(window.innerWidth < 768) { 
                document.getElementById('bottomInfoCard').classList.remove('translate-y-full');
                document.getElementById('mobileInfoToggleBtn').classList.add('hidden');
            }
        }
        ...
        function toggleSearchDropdown(show) {
            ...
            else { setTimeout(() => dropdown.classList.add('hidden'), 200); }
        }
        ...
        function simulateLocationSelect(locationKey, warningLevel = 'CRITICAL') {
            ...
            sensorMarker = L.marker([targetCoords.lat, targetCoords.lng], {icon: activeIcon})
                            .bindPopup(`<b>${warningLevel} ALERT</b><br>ESP32 Node: ` + locationKey.replace(/_/g, ' '))
                            .addTo(map).openPopup();

            calculateRouteTrail(locationCoordinatesMatrix.cdrrmo_hq, targetCoords, locationKey, warningLevel);
        }
       ...
       function calculateRouteTrail(origin, destination, key, warningLevel = 'CRITICAL') {
        ...
        function renderStatusHUDCard(key, warningLevel) {
            ...
            if (warningLevel === 'CRITICAL') {
                ...
                instructionText.className = "text-xs font-bold text-slate-600 leading-relaxed border-l-4 border-red-500 pl-3 bg-red-50/50 py-2.5 rounded-r-xl";
                ...
                instructionText.className = "text-xs font-bold text-slate-600 leading-relaxed border-l-4 border-orange-500 pl-3 bg-orange-50/50 py-2.5 rounded-r-xl";
                ...
                instructionText.className = "text-xs font-bold text-slate-600 leading-relaxed border-l-4 border-green-500 pl-3 bg-green-50/50 py-2.5 rounded-r-xl";
